...
import { getReservoirExecuteData, trackReservoirTransaction } from '@/lib/services/reservoirTx';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'tx_callback' && txHash) {
      ...
      const { trackReservoirTransaction } = await import('@/lib/services/reservoirTx');
      ...
      if (txStatus.status === 'completed') {
        ...
        return new NextResponse(
          ...
          <html>
            <head>
              <meta property="fc:frame" content="vNext">
              <meta property="fc:frame:image" content="/success_transaction.png">
        ...
        return new NextResponse(
          ...
          <html>
            <head>
              <meta property="fc:frame" content="vNext">
              <meta property="fc:frame:image" content="/failed_transaction.png">
        ...
        return new NextResponse(
          ...
          <html>
            <head>
              <meta property="fc:frame" content="vNext">
              <meta property="fc:frame:image" content="/pending_transaction.png">
    ...
    // Instead of redirecting, return a frame response with information about how to buy
    ...
    try {
      ...
      // Return a frame with two options: direct purchase via tx or open in marketplace
      const hasTxDetails = nft.market?.floorAsk?.id && 
                        nft.market?.floorAsk?.price && 
                        nft.token?.contract && 
                        nft.token?.tokenId;
      
      const chainId = networkId === '1' ? '1' : '8453'; // ETH or Base
      
      // Se tivermos todos os detalhes necessários para uma transação e o botão 1 foi pressionado
      if (hasTxDetails && buttonId === '1') {
        try {
          // Preparar os dados de execução da compra usando nosso serviço
          const orderId = nft.market?.floorAsk?.id;
          
          // Use the service to get transaction execution data
          const txResult = await getReservoirExecuteData(network, {
            orderId: orderId,
            quantity: 1,
            // If we have the user's address, use it; otherwise use the ${fid} placeholder for Warpcast substitution
            taker: userAddress || "${fid}",
            source: "pull2base", // Identify our application
            referrer: baseUrl // For referral fees if configured
          });
          ...
          // Extrair URL de transação
          const txUrl = txResult.txUrl;
          ...
          return new NextResponse(
            ...
            <html>
              <head>
                <meta property="fc:frame" content="vNext">
                <meta property="fc:frame:title" content="Buy ${name} with Wallet">
                <meta property="fc:frame:image" content="${image}">
                <meta property="fc:frame:post_url" content="${baseUrl}/api/frames/nft/action">
                
                <meta property="fc:frame:button:1" content="Sign Purchase (${price} ${currency})">
                ...
                <meta property="fc:frame:state" content="${Buffer.from(JSON.stringify({
                  ...
                  contract,
                  tokenId,
                  action: 'tx_callback', // This will trigger our callback handler when the user returns
                  image: image,
                  name: name
                })).toString('base64')}">
              </head>
      ...
      // Fallback para o fluxo normal se não conseguirmos preparar uma transação
      return new NextResponse(
        `<!DOCTYPE html>
        <html>
          <head>
